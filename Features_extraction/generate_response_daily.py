...
output_file = "E:\\Research Work\\Sentiment Analysis Project\\Dataset\\Full_Dataset\\dataset_with_senti_deca_labels\\response_daily_4.csv"
    ...
    for line in file.readlines():
      try:
          ...
          for project in project_list:
            ...
            load_dict['PR_Date_Created_At'] = datetime.strptime(str(load_dict['Timeline'][0]['Created_At'])[2:-2],
                                                                "%Y-%m-%dT%H:%M:%SZ").date()
            load_dict['PR_Time_Create_At'] = datetime.strptime(str(load_dict['Timeline'][0]['Created_At'])[2:-2],
                                                               "%Y-%m-%dT%H:%M:%SZ").time()
            load_dict['PR_Date_Closed_At'] = datetime.strptime(str(load_dict['Timeline'][1]['Created_At'])[2:-2],
                                                               "%Y-%m-%dT%H:%M:%SZ").date()
            load_dict['PR_Time_Closed_At'] = datetime.strptime(str(load_dict['Timeline'][1]['Created_At'])[2:-2],
                                                               "%Y-%m-%dT%H:%M:%SZ").time()
            ...
            Wait_Time = {}
            # print(count)
            time_length = int((time.mktime(
                time.strptime(str(load_dict['Timeline'][1]['Created_At'])[2:-2], "%Y-%m-%dT%H:%M:%SZ")) - time.mktime(
                time.strptime(str(load_dict['Timeline'][0]['Created_At'])[2:-2], "%Y-%m-%dT%H:%M:%SZ"))) / 3600 / 24)

            print("time_lenght in days: {}".format(time_length))

            for item in range(1,len(load_dict['Timeline'])):
              tmp_list = eval(str(load_dict['Timeline'][item]['Created_At']))
              for i in tmp_list:
                Wait_Time[int((time.mktime(time.strptime(i,"%Y-%m-%dT%H:%M:%SZ"))-time.mktime(time.strptime(str(load_dict['Timeline'][0]['Created_At'])[2:-2],
                                                                                                            "%Y-%m-%dT%H:%M:%SZ")))/3600/24/7)] = True
            ...
            for wait_time in range(time_length):
                # print('wait_time in loop {}'.format(wait_time))
                load_dict['wait_time_up'] = wait_time
                time_frame_started_at = (time.mktime(time.strptime(str(load_dict['Timeline'][0]['Created_At'])[2:-2].split('T')[0]+
                                                           'T23:59:59Z', "%Y-%m-%dT%H:%M:%SZ"))+3600*24*wait_time)
                pr_creation_time = (time.mktime(time.strptime(str(load_dict['Timeline'][0]['Created_At'])[2:-2], "%Y-%m-%dT%H:%M:%SZ")))
                load_dict['PR_age'] = (time_frame_started_at - pr_creation_time)/60
                load_dict['PR_response'] = False
                load_dict['Day'] = ((time.strptime(str(load_dict['Timeline'][0]['Created_At'])[2:-2],
                                                   "%Y-%m-%dT%H:%M:%SZ"))[6] + wait_time) % 7
                if wait_time in Wait_Time.keys():
                    load_dict['PR_response'] = True
