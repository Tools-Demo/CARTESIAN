...
df = pd.read_csv("../Dataset/response.csv", sep=",", encoding="ISO-8859-1")
...
for name, value in classifiers.items():
    for project in project_list:
        ...
        df_proj = df_proj[['Project_Age', 'Project_Accept_Rate', 'Language', 'Watchers', 'Stars', 'Team_Size', 'Additions_Per_Week',
                 ...
                 'Forks_Count',
                 'File_Touched_Average', 'Merge_Latency', 'Rebaseable', 'Intra_Branch', 'Project_Domain', 
                 'Additions', 'Deletions', 'Day', 'Commits_PR', 'Wait_Time', 'Contain_Fix_Bug', 'PR_Latency',
                 'Files_Changed',
                 'Label_Count', 'Assignees_Count', 'Workload', 'PR_age',
                 ...
                 'first_response', 'latency_after_first_response', 'X1_0', 'X1_1', 'X1_2', 'X1_3', 'X1_4', 'X1_5', 'X1_6',
                 'X1_7', 'X1_8', 'X1_9',
                 'PR_response', 'PR_Date_Created_At',
                 'PR_Time_Create_At', 'PR_Date_Closed_At', 'PR_Time_Closed_At']]
        ...
        X_test = df_proj.loc[
            (df_proj['PR_Date_Created_At'] >= start_date) & (df_proj['PR_Date_Created_At'] <= end_date)]
        y_test = X_test[target]
        X_train = df_proj.loc[(df_proj['PR_Date_Created_At'] < start_date)]
        y_train = X_train[target]

        print('Train dataset has a size of {}'.format(len(X_train)))
        print('Test dataset has a size of {}'.format(len(X_test)))
...
results.to_csv('Results/response_projects_results.csv', sep=',', encoding='utf-8', index=False)
